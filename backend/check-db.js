...
async function checkPayslips() {
  ...
  // Check payroll_documents collection
  const payrollDocsCount = await db.collection('payroll_documents').countDocuments({});
  console.log(`\nTotal payroll_documents: ${payrollDocsCount}`);
  
  // Get sample payroll_documents
  const samplePayrollDocs = await db.collection('payroll_documents').find({}).limit(3).toArray();
  console.log('\nSample payroll_documents:');
  samplePayrollDocs.forEach(p => {
    console.log({
      _id: p._id,
      userId: p.userId,
      year: p.year,
